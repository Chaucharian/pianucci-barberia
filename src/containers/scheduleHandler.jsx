...
const ScheduleHandler = (props) => {
    ...
    const disableDates = (date) =>
        date < new Date().getTime() || date > END_DATE
        ...
        const afternoonTimeToUnix = {
            from:
                afternoonFrom === ''
                    ? 0
                    : new Date().setHours(
                          Number(afternoonFrom.split(':')[0]),
                          0
                      ),
            to: new Date().setHours(Number(afternoonTo.split(':')[0]), 0),
        }

        api.setAvailableHours({
                    ...
                    {showCalendar && (
                        <div className={classes.calendar}>
                            <Calendar
                                ...
                                disableDates={(date) =>
                                    disableDates(addDays(date, 1))
                                }
                                onChange={(date) =>
                                    daySelection(addDays(date, 1), true)
                                }
                            />
                        </div>
                    )}

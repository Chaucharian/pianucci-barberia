...
export const BookingDateSelector = (props) => {
    ...
    const changeCurrentDate = ({
        ...
        setState({
            ...state,
            currentDate: date,
            currentDateFormated: dateFormated,
            showBookings,
            comesFromCalendar,
        })
    }
    ...
    const todayEnd = (bookings) => {
        ...
        if (bookings.length !== 0 && isToday(currentDate)) {
            dayEnded =
                ...
                getHours(bookings[bookings.length - 1].date)
        }
        return dayEnded
    }
        ...
        if (isDateDisabled(daysOff, currentDate)) {
            ...
            api.getSchedule(currentDate).then(({ bookings }) => {
                ...
                if (todayEnd(bookings)) {
                    dispatch(
                        appActions.setDaysOff([...daysOff, getDay(currentDate)])
                    )
                }
            })
        }
    ...
    return (
        <div className={classes.container}>
            <DaysListSelector
                ...
                daysOff={daysOff}
                onDaySelected={changeCurrentDate}
                onOpenCalendar={openCalendar}
            />
            <Spinner loading={fetching && showBookings}>
                {showBookings && comesFromCalendar && (
                    <h3 className={classes.dateIndicator}>
                        TURNOS PARA {currentDateFormated}
                    </h3>
                )}
